...
void buildHTTPheader(char buffer[],char* host,char* path){
...
int fresh(int index){
	time_t nw ;
    time(&nw);
	if (CacheTable[index].Expires != NULL && mktime(CacheTable[index].Expires)-nw > 0){
        return 1;
	}
	else if(CacheTable[index].Date!=NULL && CacheTable[index].Last_Modified!=NULL && nw-mktime(CacheTable[index].Date)<= ONEDAY && nw-mktime(CacheTable[index].Last_Modified) <= ONEMONTH){
		return 1;
	}
	return 0; 
}
...
int processing(int clientsocket,struct Cache CacheTable[],int* CurrentCacheNum,char* host,char* path){
	...
	if (index != -1){     // if there is entry in the cache for this URL
        if (fresh(index)==1){   // the entry is still valid,just return this cache entry
        	printf("this website is cached and not staled, so transmit it to client\n");
        ...
        buildHTTPheader(request,host,path);
		//printf("hello123\n");
		...
		if (flag == 0){
        	printf("the responce does not have expiry and last-modified header, do not cache it .\n");
        }
